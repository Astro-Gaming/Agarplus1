function MyApp() {
    ...
    this.init = function() {
        ...
        conn = new Connection;
        conn.setCurrentServer();// conn.connect();
    ...
    this.afterGameLoaded = function() {
        ...
        $("#ip_info").text("Server: " + myApp.getCurrentIP());*/ // Zero lo tenía oculto
    ...
    this.setupHotKey = function() {
        hotkeyConfig = {
            hk_start_new_game: {
                ...
                name: "Start new game",
                keyDown: function() {
					setCurrentServer($("#chooseServer").val()); // establece el servidor al que nos conectaremos
...
function Connection() {
    var msg;
    var self = this;
    self.setCurrentServer = function() {//self.connect = function() {
        socket = io("149.202.209.118:9700", {//socket = io("minimap.alis.io:8001", { // IP del chat/minimapa
    ...
    function run() {
        ...
        /*jQuery.get("http://m.agar.io/info", function(b) {
            ...
            for (name in testSource) {
                jQuery('#region option[value="' + name + '"]').text(old[name] + " (" + testSource[name] + " players)");
            }
        }, "json");*/
    }
    ...
    if (socket.on("playerUpdated", function(p) {
        ...
        if (ee && -1 == self.location.search.indexOf("fb")) {
            ...
            if ("undefined" == typeof console || ("undefined" == typeof DataView || ("undefined" == typeof WebSocket || (null == test_canvas || (null == test_canvas.getContext || null == self.localStorage))))) {
                ...
                var save = null;
                self.setCurrentServer = open;//self.connect = open; //
                var backoff = 500;
                var tref = null;
                var millis = 0;
                var maxX = -1;
